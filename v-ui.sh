...
log_folder="${XUI_LOG_FOLDER:=/var/log/v-ui}"
mkdir -p "${log_folder}"
...
update() {
    confirm "This function will update all v-ui components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rc-update del v-ui
        rm /etc/init.d/v-ui -f
    else
        systemctl stop v-ui
        systemctl disable v-ui
        rm ${xui_service}/v-ui.service -f
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${xui_folder}/v-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor false > /dev/null 2>&1
    else
        ${xui_folder}/v-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the V-UI panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${xui_folder}/v-ui setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${xui_folder}/v-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${xui_folder}/v-ui setting -show true)
    if [[ $? != 0 ]]; then
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${xui_folder}/v-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "v-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "v-ui and xray stopped successfully"
        else
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        LOGI "v-ui and xray Restarted successfully"
    else
...
status() {
    if [[ $release == "alpine" ]]; then
        rc-service v-ui status
    else
        systemctl status v-ui -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    if [[ $release == "alpine" ]]; then
        rc-update add v-ui default
    else
        systemctl enable v-ui
    fi
    if [[ $? == 0 ]]; then
        LOGI "v-ui Set to boot automatically on startup successfully"
    else
        LOGE "v-ui Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "v-ui Autostart Cancelled successfully"
    else
        LOGE "v-ui Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u v-ui -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-v-ui.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-v-ui.conf | tr -d '#')
        sysctl -w net.core.default_qdisc="${old_settings%:*}"
        sysctl -w net.ipv4.tcp_congestion_control="${old_settings#*:}"
        rm /etc/sysctl.d/99-bbr-v-ui.conf
        sysctl --system
    else
...
check_status() {
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $(rc-service v-ui status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
        ...
        temp=$(systemctl status v-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'v-ui' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled v-ui)
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${xui_folder}/v-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart v-ui 2>/dev/null || rc-service v-ui restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="v-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}v-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; v-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; v-ui restart"
                reloadCmd="systemctl reload nginx ; v-ui restart"
                ;;
            2)
                LOGD "It's recommended to put v-ui restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; v-ui restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${xui_folder}/v-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${xui_folder}/v-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${xui_folder}/v-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${xui_folder}/v-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${xui_folder}/v-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${xui_folder}/v-ui setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${xui_folder}/v-ui setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
|  ${blue}v-ui restart-xray${plain}          - Restart Xray                     │
...
│  ${blue}v-ui update-all-geofiles${plain}   - Update all geo files             │
