...
func (s *ServerService) GetLogs(count string, level string, syslog string) []string {
	...
	if syslog == "true" {
		...
		cmd := exec.Command("journalctl", "-u", "v-ui", "--no-pager", "-n", strconv.Itoa(countInt), "-p", level)
		var out bytes.Buffer
		cmd.Stdout = &out
		err = cmd.Run()
		if err != nil {
			return []string{"Failed to run journalctl command! Make sure systemd is available and v-ui service is registered."}
		}
