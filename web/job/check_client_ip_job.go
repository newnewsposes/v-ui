		...
		if runtime.GOOS == "windows" {
			...
			if iplimitActive {
				if f2bInstalled {
					...
					if !f2bInstalled {
						logger.Warning("[LimitIP] Fail2Ban is not installed, Please install Fail2Ban from the v-ui bash menu.")
					}
				}
			}
		}
...
func (j *CheckClientIpJob) updateInboundClientIps(inboundClientIps *model.InboundClientIps, clientEmail string, newIpsWithTime []IPWithTimestamp) bool {
	...
	if len(liveIps) > limitIp {
		...
		// log format is load-bearing: v-ui.sh create_iplimit_jails builds
