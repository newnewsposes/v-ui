...
### Database Management
- DB path: Configured via `config.GetDBPath()`, typically `/etc/v-ui/v-ui.db`
...
Both `install.sh` and `v-ui.sh` follow these patterns:
...
- Binary: `/usr/local/v-ui/`
- Database: `/etc/v-ui/v-ui.db`
- Logs: `/var/log/v-ui/`
- Service: `/etc/systemd/system/v-ui.service.*`

## Testing & Debugging
- Set `XUI_DEBUG=true` for detailed logging
- Check Xray process: `v-ui.sh` script provides menu for status/logs
- Database inspection: Direct SQLite access to v-ui.db
